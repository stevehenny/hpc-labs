...
static void create_dataset(int datasetNum, int numARows, int numACols,
  ...
  const char *dir_name =
      htkDirectory_create(htkPath_join(base_dir, datasetNum));

  char *input0_file_name = htkPath_join(dir_name, "input0.raw");
  char *input1_file_name = htkPath_join(dir_name, "input1.raw");
  char *output_file_name = htkPath_join(dir_name, "expect.raw");
